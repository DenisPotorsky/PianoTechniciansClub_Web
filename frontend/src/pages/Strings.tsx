...
const Strings: React.FC = () => {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewRecord({
        brand: '', model: '', chor_nummer: 0, saiten_im_chor: '',
        laenge_mm: '', kern_mm: 0, erste_wicklung_mm: '',
    ...
    <div className="max-w-7xl mx-auto">
      <div className="glass-card p-4 md:p-6 lg:p-8">
        <div className="text-center mb-6 md:mb-8">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-white">Мензуры струн</h2>
          <p className="text-white/50 mt-1 text-sm md:text-base">Данные по длинам и диаметрам струн</p>
        </div>
        ...
        {/* Форма добавления */}
        {showAddForm && isAdmin && (
          <form onSubmit={editingId ? handleUpdate : handleAdd} className="mb-6 glass p-4 md:p-6 rounded-2xl">
            ...
            <button
              type="submit"
              className="mt-4 glass-btn glass-btn-primary w-full sm:w-auto"
            >
        ...
        {loading ? (
          ...
          <div className="relative">
            <div className="overflow-x-auto glass-table" style={{WebkitOverflowScrolling: 'touch'}}>
              <table className="min-w-full divide-y divide-white/5 text-xs sm:text-sm whitespace-nowrap">
                <thead>
                  <tr>
                    <th className="px-3 py-3 text-left font-medium text-white/40">№</th>
                    <th className="px-3 py-3 text-left font-medium text-white/40">Бренд</th>
                    <th className="px-3 py-3 text-left font-medium text-white/40">Модель</th>
                    <th className="px-3 py-3 text-left font-medium text-white/40">Хор</th>
                    <th className="px-3 py-3 text-left font-medium text-white/40">Струн</th>
                    <th className="px-3 py-3 text-left font-medium text-white/40">Длина</th>
                    <th className="px-3 py-3 text-left font-medium text-white/40">Керн</th>
                    <th className="px-3 py-3 text-left font-medium text-white/40">1-я нав.</th>
                    <th className="px-3 py-3 text-left font-medium text-white/40">2-я нав.</th>
                    <th className="px-3 py-3 text-left font-medium text-white/40">Тип</th>
                    <th className="px-3 py-3 text-left font-medium text-white/40">Год</th>
                    {isAdmin && <th className="px-3 py-3 text-left font-medium text-white/40">Действия</th>}
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {data.map((row) => (
                    <tr key={row.id} className="hover:bg-white/5 transition">
                      <td className="px-3 py-3 text-white/60">{row.id}</td>
                      <td className="px-3 py-3 font-medium text-white">{row.brand}</td>
                      <td className="px-3 py-3 text-white/80">{row.model}</td>
                      <td className="px-3 py-3 text-white/80">{row.chor_nummer}</td>
                      <td className="px-3 py-3 text-white/80">{row.saiten_im_chor || '—'}</td>
                      <td className="px-3 py-3 text-white/80">{row.laenge_mm || '—'}</td>
                      <td className="px-3 py-3 font-semibold text-indigo-300">{row.kern_mm}</td>
                      <td className="px-3 py-3 text-white/80">{row.erste_wicklung_mm || '—'}</td>
                      ...
                      <td className="px-3 py-3 text-white/80">{row.typ || '—'}</td>
                      <td className="px-3 py-3 text-white/80">{row.year || '—'}</td>
                      {isAdmin && (
            ...
            <p className="text-center text-white/40 text-xs mt-2">← Проведите для просмотра всех колонок →</p>
          </div>
        )}
      </div>
    </div>
